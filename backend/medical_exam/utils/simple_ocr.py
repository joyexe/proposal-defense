...
class SimpleOCRProcessor:
    """
    Simple OCR processor using only PIL and pytesseract
    Fallback when OpenCV is not available or has issues
    """
    
    def __init__(self):
        # Optimized OCR configs for faster processing
        self.ocr_configs = [
    ...
    def preprocess_image(self, image_path):
        """
        Apply optimized preprocessing using PIL for faster processing
        """
        try:
            # Open image with PIL
            ...
            # 2. Grayscale (most effective for OCR)
            gray_img = img.convert('L')
            processed_images.append(gray_img)
            
            # 3. Enhanced contrast grayscale
            ...
            logger.error(f"Error preprocessing image with PIL: {e}")
            return []
    
    def extract_text_with_multiple_configs(self, image_path):
        """
        Extract text using multiple OCR configurations and preprocessing techniques
        """
        best_result = ""
        best_confidence = 0
        ...
        try:
            ...
            if not processed_images:
                logger.error("No images could be processed with PIL")
                return ""
...
class SimpleMedicalFormParser:
    """
    Simple parser for medical examination forms
    Uses the same parsing logic as the enhanced version
    """
    ...
    def extract_medical_data(self, image_path):
        ...
        try:
            ...
            return {
                ...
                "processing_method": "Simple OCR with PIL"
            }
    ...
    def parse_medical_form(self, raw_text):
        ...
        Same parsing logic as the enhanced version
        """
        data = {}
...
def extract_medical_data_from_image_simple(image_path):
    """
    Convenience function to extract medical data from image using simple OCR
    """
    parser = SimpleMedicalFormParser()
