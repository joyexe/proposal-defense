...
import { getAppointments, createAppointment, getProviders, getProviderAvailableTimes, getUserProfile, cancelAppointment } from '../../utils/api';
...
export default function StudentAppointmentPage() {
  ...
  useEffect(() => {
    ...
    // Fetch user profile for referral logic
    getUserProfile().then(setUserProfile);
  }, []);

  // Fetch available times when provider changes
  useEffect(() => {
    if (providerId) {
  ...
  // When scheduling a new appointment, call createAppointment and refresh list
  // Hindi na kailangan, real-time na ang update
  ...
  function ScheduleAppointmentModal({
    show, onClose, onScheduled, appointments, setAppointments }) {
    const [providerId, setProviderId] = useState("");
    const [date, setDate] = useState("");
    const [time, setTime] = useState("");
    const [notes, setNotes] = useState("");
          ...
          <li className="nav-item flex-fill">
            <button
              ...
              onClick={() => {
                ...
                router.push('/student/appointment/schedule-lists');
              }}
                            ...
                            <>
                              ...
                              {getAppointmentsForDay(
                                ...
                                .map((appt, idx) => {
                                  ...
                                  if (appt.provider_id) {
                                    providerInfo = providers.find(p => String(p.id) === String(appt.provider_id));
                                  } else if (appt.provider && appt.provider.id) {
                                    providerInfo = providers.find(p => String(p.id) === String(appt.provider.id));
                                  }
                                  ...
                                  if (providerRole.toLowerCase() === 'clinic') providerRole = 'Nurse';
                                  else if (providerRole.toLowerCase() === 'faculty') providerRole = 'Teacher';
                                  else if (providerRole) providerRole = providerRole.charAt(0).toUpperCase() + providerRole.slice(1);
                                  const displayText = `${to12Hour(stripSeconds(appt.time))} | With ${providerRole ? providerRole + ' ' : ''}${providerName}`;
                                  let bgColor = '#bbf7d0';
                                  let textColor = '#222';
