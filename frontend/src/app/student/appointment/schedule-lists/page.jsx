...
import { getUserProfile, getAppointments, createAppointment, getProviders, getProviderAvailableTimes } from '../../../utils/api';

const typeOptions = ["Type", "Physical Health", "Mental Health"];
const statusOptions = ["Status", "Upcoming", "Completed", "Cancelled", "In Progress"];

// Helper functions for time formatting (make available globally in this file)
...
// Helper to get provider name for viewedAppointment
function getViewedProviderName(viewedAppointment, providers) {
  if (!viewedAppointment) return '-';
  let providerInfo = null;
  if (viewedAppointment.provider_id) {
    providerInfo = providers.find(p => String(p.id) === String(viewedAppointment.provider_id));
  } else if (viewedAppointment.provider && viewedAppointment.provider.id) {
    providerInfo = providers.find(p => String(p.id) === String(viewedAppointment.provider.id));
  }
  if (providerInfo && typeof providerInfo.full_name === 'string') return providerInfo.full_name;
  if (viewedAppointment.provider && typeof viewedAppointment.provider.full_name === 'string') return viewedAppointment.provider.full_name;
  if (typeof viewedAppointment.provider === 'string') return viewedAppointment.provider;
  return 'Provider';
}

// Helper to get provider name and role
...
// Helper to get referral label
function getReferralLabel(appt, userId) {
  // If the current user created the appointment, show 'Direct'
  if (appt.created_by && userId && appt.created_by.id === userId) {
    return 'Direct';
  }
  // If created by someone else, show role-based label
  if (appt.created_by && appt.created_by.id !== (appt.client && appt.client.id)) {
    const name = appt.created_by.full_name || 'User';
    let roleLabel = '';
    if (appt.created_by.role === 'clinic') roleLabel = 'Nurse';
    else if (appt.created_by.role === 'counselor') roleLabel = 'Counselor';
    else if (appt.created_by.role === 'faculty') roleLabel = 'Teacher';
    else if (appt.created_by.role === 'student') roleLabel = 'Student';
    else if (appt.created_by.role === 'admin') roleLabel = 'Admin';
    else if (appt.created_by.role) roleLabel = appt.created_by.role.charAt(0).toUpperCase() + appt.created_by.role.slice(1);
    if (roleLabel) {
      return `From ${roleLabel} ${name}`;
    } else {
      return `From ${name}`;
    }
  }
...
function getProviderDisplayName(provider) {
  ...
  if (typeof provider === 'object') {
    ...
    if (provider.role) {
      if (provider.role === 'student') role = 'Student';
      else if (provider.role === 'faculty') role = 'Teacher';
      else if (provider.role === 'clinic') role = 'Nurse';
      else if (provider.role === 'counselor') role = 'Counselor';
      else if (provider.role === 'admin') role = 'Admin';
      else role = provider.role.charAt(0).toUpperCase() + provider.role.slice(1);
    }
...
export default function ScheduleListsPage() {
  ...
  const handleSchedule = async (e) => {
    e.preventDefault();
    try {
      await createAppointment({ provider, date, time, reason: notes });
      setShowModal(false);
      setProvider(""); setDate(""); setTime(""); setNotes("");
  ...
  return (
    <div className="w-100" style={{ minHeight: '100vh', background: '#f8fafc', minWidth: 0 }}>
        ...
        <ul className="nav nav-pills w-100">
          <li className="nav-item flex-fill">
            <button
              ...
              onClick={() => router.push('/student/appointment')}
      ...
      <div className="bg-white rounded shadow-sm p-4" style={{ borderRadius: 16, marginTop: 0, minHeight: 350 }}>
              ...
              {paginatedAppointments.length === 0 ? (
                ...
                paginatedAppointments
                  .map((appt, idx) => {
                    ...
                    const providerName = providerInfo && typeof providerInfo.full_name === 'string' ? providerInfo.full_name : (appt.provider && typeof appt.provider.full_name === 'string' ? appt.provider.full_name : typeof appt.provider === 'string' ? appt.provider : 'Provider');
                    const providerRole = providerInfo && typeof providerInfo.role === 'string' ? providerInfo.role : (appt.provider && typeof appt.provider.role === 'string' ? appt.provider.role : '');
                    const providerRoleLabel = providerRole ? `(${providerRole.charAt(0).toUpperCase() + providerRole.slice(1)})` : '';
                    const displayText = `${to12Hour(stripSeconds(appt.time))} | With ${providerName} ${providerRoleLabel}`;
                    const referralText = typeof appt.referral === 'string' ? appt.referral : '-';
                    return (
                  <tr key={idx}>
                        <td style={{ border: 'none' }}>{getProviderDisplayName(appt.provider)}</td>
        ...
        {/* Pagination Controls with ellipsis and page numbers, system logs style */}
        {totalPages > 1 && (
              ...
              <div className="row mb-2">
                <div className="col-6">
                  <div className="fw-semibold mb-1" style={{ fontSize: 15 }}>
                    ...
                    {getProviderDisplayName(viewedAppointment.provider)}
                  </div>
                </div>
                {/* Type */}
                <div className="col-6">
                ...
                <div className="col-12">
                  ...
                  <div style={{ fontSize: 14 }}>
                    ...
                    {(() => {
                      ...
                      // Try to find provider from providers list if not present
      ...
      {/* Modal for scheduling a new appointment */}
      {showModal && (
