...
// Add helper to wrap images in a container
...
export default function StudentBulletinPage() {
  ...
  // Carousel logic: show 3 at a time horizontally
  ...
  const handlePrev = () => {
    if (canGoPrev) setCarouselIndex(carouselIndex - 1);
  };
  const handleNext = () => {
    if (canGoNext) setCarouselIndex(carouselIndex + 1);
  };
      ...
      <div className="d-flex align-items-center mb-4">
        <ul className="nav nav-pills w-100">
          <li className="nav-item flex-fill">
            <button
              className={`nav-link w-100 border ${pathname === '/student/bulletin' ? 'bg-success text-white' : 'bg-light text-dark'}`}
              onClick={() => router.push('/student/bulletin')}
          ...
          <li className="nav-item flex-fill">
            <button
              className={`nav-link w-100 border ${pathname === '/student/bulletin/bulletin-posts' ? 'bg-success text-white' : 'bg-light text-dark'}`}
              onClick={() => router.push('/student/bulletin/bulletin-posts')}
