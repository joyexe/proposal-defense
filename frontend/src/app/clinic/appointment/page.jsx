...
import { getAppointments, createAppointment, getStudents, getProviders, getProviderAvailableTimes, getUserProfile, updateAppointment, markAppointmentCompleted, markAppointmentInProgress, cancelAppointment, detectICD11Realtime, searchICD11Codes, updateAppointmentDocumentation, testICD11Backend } from '../../utils/api';
...
export default function ClinicAppointmentPage() {
  ...
  // Fetch students and providers when Refer modal opens
  useEffect(() => {
    if (showReferModal) {
  ...
  // Fetch available times for selected provider
  useEffect(() => {
    if (referTo) {
  ...
  function getAppointmentsForDay(dateStr) {
    return appointments.filter((a) => a.date === dateStr)
      .filter((a) => {
        // Robust ID comparison (string/number)
      ...
      // If logged-in user is clinic and created the appointment, show all
        if (userProfile && userProfile.role === 'clinic' && userId && createdById && userId === createdById) {
        return true;
      }
      // Otherwise, only show Physical Health
      return (
        (a.service_type && a.service_type.toLowerCase() === 'physical') ||
        (typeof a.label === 'string' && a.label.toLowerCase().includes('physical health'))
      );
      })
  ...
  function getTypeByLabel(label) {
    ...
    if (label.toLowerCase().includes('consult')) return 'Consultation';
    if (label.toLowerCase().includes('checkup')) return 'Checkup';
    return '';
  }
  ...
  const detectICD11Conditions = async () => {
    ...
    try {
      ...
      const response = await detectICD11Realtime(
        studentReason, 
        nurseDocumentation, 
        {}, // No vital signs for appointments
        'appointment'
      );
                          ...
                          {date && (
                            <>
                              <div className={`fw-semibold ${date.getMonth() === currentMonth ? '' : 'text-muted'}`}>{date.getDate()}</div>
                                {getAppointmentsForDay(getLocalDateString(date)).map((appt, idx) => {
                                // Find client info
                                ...
                                const clientName = client.full_name || 'Unknown';
                                  ...
                                  const userId = userProfile && userProfile.id ? String(userProfile.id) : null;
                                  const createdById = appt.created_by && appt.created_by.id ? String(appt.created_by.id) : null;
                                  // Display logic
                                  let displayText;
                                  if (
                                    appt.service_type && appt.service_type.toLowerCase() === 'mental' &&
                                    userProfile && userProfile.role === 'clinic' && userId && createdById && userId === createdById
                                  ) {
                                    displayText = `${to12Hour(appt.time)} | For Student ${clientName}`;
                                  } else {
          ...
          <div className="modal-dialog modal-dialog-centered" style={{ maxWidth: 540, minWidth: 540, width: "100%" }}>
            <div className="modal-content modal-light-green-background p-4" style={{ borderRadius: 16, boxShadow: "0 4px 32px rgba(0,0,0,0.10)", background: "#e6f0ea", minHeight: 520 }}>
              ...
              <div className="row mb-2">
                ...
                <div className="col-6">
                  ...
                  <div style={{ fontSize: 14 }}>
                    {/* Type badge style from List of Schedules */}
                    {(() => {
              ...
              <div className="d-flex justify-content-between align-items-center gap-2">
                ...
                <button type="button" className="btn btn-teal px-4 d-flex align-items-center" style={{ background: '#14b8a6', color: '#fff', fontWeight: 600, border: 'none' }} onClick={() => {
                  setShowAddNotesModal(true);
                  // Load existing documentation and diagnosis if available
                  if (selectedAppointment?.documentation) {
                    setAdditionalNotes(selectedAppointment.documentation);
                  }
                  if (selectedAppointment?.diagnosis_code && selectedAppointment?.diagnosis_name) {
                    setSelectedDiagnosis({
                      code: selectedAppointment.diagnosis_code,
                      name: selectedAppointment.diagnosis_name
                    });
                    setShowDiagnosisOptions(false);
                  }
                }}>
