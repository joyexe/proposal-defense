...
import { getStudents, getProviders, getProviderAvailableTimes, createAppointment, getAppointments, markAppointmentCompleted, markAppointmentInProgress, cancelAppointment, getUserProfile } from '../../../utils/api';
...
export default function ClinicScheduleListsPage() {
              ...
              <tbody>
                {paginatedAppointments.length === 0 ? (
                  ...
                  paginatedAppointments.map((appt, idx) => (
                    <tr key={idx}>
                      <td style={{ border: 'none' }}>{appt.client && appt.client.full_name ? `${appt.client.role ? (appt.client.role.toLowerCase() === 'clinic' ? 'Nurse ' : appt.client.role.toLowerCase() === 'faculty' ? 'Teacher ' : appt.client.role.charAt(0).toUpperCase() + appt.client.role.slice(1) + ' ') : ''}${appt.client.full_name}` : 'Unknown'}</td>
                      <td style={{ border: 'none' }}>{appt.provider && appt.provider.full_name ? `${appt.provider.role ? (appt.provider.role.toLowerCase() === 'clinic' ? 'Nurse ' : appt.provider.role.charAt(0).toUpperCase() + appt.provider.role.slice(1) + ' ') : ''}${appt.provider.full_name}` : 'Unknown'}</td>
                      ...
                      <td style={{ border: 'none' }}>{getTypeBadge(appt.service_type ? (appt.service_type.toLowerCase() === 'physical' ? 'Physical Health' : appt.service_type.toLowerCase() === 'mental' ? 'Mental Health' : appt.service_type) : '')}</td>
                      <td style={{ border: 'none' }}>{getStatusBadge(appt.status)}</td>
                      <td style={{ border: 'none' }}>
                        {(() => {
                          if (!appt.created_by || typeof appt.created_by !== 'object' || !userProfile) return '';
                          const isDirect = userProfile.role === 'clinic' && String(appt.created_by.id) === String(userProfile.id);
            ...
            <div className="modal-content modal-light-green-background p-4" style={{ borderRadius: 16, boxShadow: "0 4px 32px rgba(0,0,0,0.10)", background: "#e6f0ea" }} ref={modalRef}>
              <div className="mb-3">
                <span className="fw-bold" style={{ fontSize: 20 }}>
                  {viewedAppointment.service_type && (viewedAppointment.service_type.toLowerCase() === 'physical' ? 'Annual Physical' : viewedAppointment.service_type.toLowerCase() === 'mental' ? 'Annual Mental Health' : 'Appointment')}
                </span>
              ...
              <div className="row mb-2">
                <div className="col-6">
                  ...
                  <div style={{ fontSize: 14 }}>
                    ...
                    {viewedAppointment.date ? new Date(viewedAppointment.date).toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }) : '-'}
                  </div>
                  <div style={{ fontSize: 14 }}>
                    <i className="bi bi-clock me-2"></i>
                    {viewedAppointment.time ? to12Hour(viewedAppointment.time) : '-'}
                  </div>
                </div>
                <div className="col-6">
                  <div className="fw-semibold mb-1" style={{ fontSize: 15 }}>Type</div>
                  <div style={{ fontSize: 14 }}>{getTypeBadge(viewedAppointment.service_type ? (viewedAppointment.service_type.toLowerCase() === 'physical' ? 'Physical Health' : viewedAppointment.service_type.toLowerCase() === 'mental' ? 'Mental Health' : viewedAppointment.service_type) : '')}</div>
                </div>
              </div>
              {/* Status and Referral */}
              <div className="row mb-2">
                ...
                <div className="col-6">
                  ...
                  <div style={{ fontSize: 14 }}>{(() => {
                    ...
                    const isDirect = userProfile.role === 'clinic' && String(viewedAppointment.created_by.id) === String(userProfile.id);
                    if (isDirect) return 'Direct';
                    let role = '';
            ...
            <div className="modal-content modal-light-green-background p-4" style={{ borderRadius: 16, boxShadow: "0 4px 32px rgba(0,0,0,0.10)", background: "#e6f0ea" }}>
              ...
              <form onSubmit={async e => {
                ...
                <div className="d-flex justify-content-end gap-2">
                  ...
                  <button type="submit" className="btn btn-success px-4" disabled={referSubmitting}>Send Referral</button>
                </div>
              </form>
            </div>
          </div>
