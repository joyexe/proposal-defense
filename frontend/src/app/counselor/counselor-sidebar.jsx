...
import "./counselor-sidebar.css";
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faGauge, faFileMedical, faCalendarCheck, faChartBar, faBullhorn, faArrowRightArrowLeft } from '@fortawesome/free-solid-svg-icons';

const links = [
  { href: "/counselor/dashboard", label: "Dashboard", icon: faGauge },
  { href: "/counselor/appointment", label: "Appointments", icon: faCalendarCheck },
  { href: "/counselor/analytics", label: "Analytics", icon: faChartBar },
  { href: "/counselor/bulletin", label: "Bulletin", icon: faBullhorn },
];

export default function CounselorSidebar({ setActiveText }) {
  ...
  const handleMenuClick = () => {
    setCollapsed(true);
    setHasClicked(true);
    localStorage.setItem("counselorSidebarCollapsed", "true");
  };
  ...
  return (
    <aside
      className={`counselor-sidebar d-flex flex-column justify-content-between p-0 ${isCollapsed ? "collapsed" : "expanded"}`}
