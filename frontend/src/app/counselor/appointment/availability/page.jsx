...
function mergeSlotsWithDefaults(backendSlots) {
  ...
  // For each default time, use backend slot if exists, else create a checked slot (no id)
  return defaultTimes.map(time => {
    const normTime = toHHMMSS(time);
    return slotMap[normTime] ? slotMap[normTime] : { time: normTime, available: true };
  });
}
...
export default function ManageAvailability() {
  ...
  const handleToggle = async (time) => {
    ...
    if (idx === -1) return; // slot not found, do nothing
    // Optimistically update UI
    const prevSlots = [...slots];
    const slot = slots[idx];
    const updatedSlot = { ...slot, available: !slot.available };
    const newSlots = slots.map((s, i) => i === idx ? updatedSlot : s);
    setSlots(newSlots);
    setSaving(true);
    try {
      if (slot.id) {
        await updateAvailability(slot.id, JSON.stringify({ available: updatedSlot.available, date: getTodayDate() }));
      } else {
        // Only create if not already present
        if (!slots.some(s => s.time === toHHMMSS(updatedSlot.time))) {
    ...
    <div className="w-100" style={{ minHeight: '100vh', background: '#f8fafc', minWidth: 0 }}>
      ...
      <div className="d-flex justify-content-between align-items-center mb-3">
        <div>
          ...
          <div className="text-muted" style={{ fontSize: 14 }}>Manage mental health appointments</div>
        </div>
