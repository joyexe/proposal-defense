...
import { faGauge, faFileMedical, faArrowRightArrowLeft, faBullhorn } from '@fortawesome/free-solid-svg-icons';

const links = [
  { href: "/faculty/dashboard", label: "Dashboard", icon: faGauge },
  { href: "/faculty/health-record", label: "Permit Requests", icon: faFileMedical },
  { href: "/faculty/appointment", label: "Appointments", icon: faArrowRightArrowLeft },
  { href: "/faculty/bulletin", label: "Bulletin", icon: faBullhorn },
];

export default function FacultySidebar({ setActiveText }) {
  ...
  const handleMenuClick = () => {
    setCollapsed(true);
    setHasClicked(true);
    localStorage.setItem("facultySidebarCollapsed", "true");
  };
  ...
  return (
    <aside
      className={`faculty-sidebar d-flex flex-column justify-content-between p-0 ${isCollapsed ? "collapsed" : "expanded"}`}
